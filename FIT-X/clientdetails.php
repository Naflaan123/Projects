...
<head>
    ...
    <title>Fit-X - Manage Client Details</title>
    <link href="adminstyle.css" rel="stylesheet" type="text/css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Poppins:wght@100;200;300;500&display=swap" rel="stylesheet">
    <style>
        .client-search, .client-details {
            display: flex;
        ...
        .client-search form, .client-details form {
            width: 300px;
            display: flex;
            align-items: center;
            flex-direction: column;
        ...
        .client-search label, .client-details label {
            font-weight: 500;
            margin-bottom: 5px;
        }
        .client-search input[type="text"], 
        .client-details input[type="text"], 
        .client-details input[type="password"] {
            width: 100%;
            padding: 8px;
            border: 1px solid #ccc;
            border-radius: 4px;
        }
        .client-search button, .report-button {
            padding: 8px;
            border: none;
            border-radius: 4px;
            color: white;
...
<body>
    ...
    <section class="client-search">
        <h2>Search for Client Details</h2><br>
        ...
        <?php
        $userid = $username = $email = $password = $gender = "";
        ...
        if (isset($_POST['search'])) {
            ...
            $sql = "SELECT userid, username, email, password, gender FROM tblclient WHERE userid = ?";
            ...
            if ($result->num_rows > 0) {
                ...
                echo "<p>No client found with User ID: " . htmlspecialchars($userid) . "</p>";
            }
        ...
        if (isset($_POST['edit'])) {
            ...
            $conn = new mysqli('localhost', 'root', '', 'fit_x');
            $stmt = $conn->prepare("UPDATE tblclient SET username=?, email=?, gender=? WHERE userid=?");
            $stmt->bind_param("ssss", $_POST['username'], $_POST['email'], $_POST['gender'], $_POST['userid']);
            $stmt->execute();
            $stmt->close();
            $conn->close();
        }
        ...
        if (isset($_POST['delete'])) {
            ...
            $conn = new mysqli('localhost', 'root', '', 'fit_x');
            $stmt = $conn->prepare("DELETE FROM tblclient WHERE userid=?");
        ...
        <!-- Display Editable Client Details -->
        <section class="client-details">
            ...
            <form action="" method="POST">
                ...
                <div class="button-container">
                    ...
                    <a href="manage_clients.php" class="report-button">Report</a> <!-- New Report button -->
                </div>
            </form>
        </section>
    </section>
